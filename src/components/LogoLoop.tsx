  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        const tooltipTitle = (item as { title?: string }).title;
        const tooltipDesc = (item as { description?: string }).description;
        const hasTooltip = tooltipTitle || tooltipDesc;
        const tooltipKey = String(key);

        return (
          <li className="logoloop__item" key={key} role="listitem">
            {hasTooltip ? (
              <span
                className={`logoloop__tooltip-wrapper${activeTooltip === tooltipKey ? ' logoloop__tooltip-wrapper--active' : ''}`}
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                {itemContent}
                <span className="logoloop__tooltip" role="tooltip">
                  {tooltipTitle && <span className="logoloop__tooltip-title">{tooltipTitle}</span>}
                  {tooltipDesc && <span className="logoloop__tooltip-desc">{tooltipDesc}</span>}
                </span>
              </span>
            ) : (
              itemContent
